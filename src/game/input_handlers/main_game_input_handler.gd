...
const inventory_menu_item_scene = preload("res://src/gui/inventory_menu/inventory_menu.tscn")
...
func get_action(player: Entity) -> Action:
	...
	if Input.is_action_just_pressed("activate"):
		var select_item: Entity = await get_item("Select an item to use", player.inventory_component)
		action = ItemAction.new(player, select_item)
...
func get_item(window_title: String, inventory: InventoryComponent) -> Entity:
	var inventory_menu: InventoryMenu = inventory_menu_item_scene.instantiate()
